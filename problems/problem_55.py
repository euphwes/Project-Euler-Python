...
far to map it to a palindrome. In fact, 10677 is the first number to be shown to require over fifty
...
def is_not_lychrel(n):
    """ Returns whether or not n is Lychrel number (described) above, within 50 iterations. """

    n = str(n)

    for _ in range(49):
        test = int(n) + int(''.join(reversed(n)))
        if is_palindome(test):
            return False
        n = str(test)
...
@time_it
def problem_55():
    print(len(list(filter(is_not_lychrel, range(1,10000)))))
